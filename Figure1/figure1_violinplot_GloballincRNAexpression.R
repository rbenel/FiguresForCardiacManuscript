...
#a global picture of lincRNA expression during the experiment 
...
lincRNAgenes <- tibble::rownames_to_column(lincRNAgenes, "id")

#long data.. with the day, count, and type 
lincRNAgenes.long <- lincRNAgenes %>% tidyr::gather(days, count, 3:ncol(lincRNAgenes))

#rearrange the factor so that it representes the "real" order not alphabetical 
lincRNAgenes.long$days <-factor(lincRNAgenes.long$days, levels=c("day0", "day1", "day2", "day3", "day4", "day5", "day6",
                                                             "day8", "day15", "day30", "day60"))
...
#facet option of violin graph
FacetOption <- ggplot(lincRNAgenes.long, aes(x = days, y = count, fill = days)) + 
              geom_violin()+
              scale_fill_manual(values = c("#f9bdbd", "#f74a4a", "#ffaa56", "#f9e67a", "#a6dd6a", "#05aa60", 
                                           "#b0e5df",  "#4bb7e5","#648cea", "#caa9e5","#9a25d1")) +
              labs(y = "RNA expression (log2)", x = "Days") + 
              # legend
              stat_compare_means(aes(label = ..p.signif..), method = "wilcox.test", 
                                 ref.group = "day0", hide.ns = TRUE) +
              #need this when the graph is not a boxplot, to add the median to the plot
              stat_summary(fun = median, geom = "point", shape = 23, size = 2) + 
              ###########################
              #legend and custom graph
              ###########################
...
ggsave("/Bigdata/Dropbox (Technion Dropbox)/Rina_Benel/Home/Rina/AS_linc/cardiac_diff/results/salmon10.1_results/figures_paper/Figure1/March22_2020_figure1/ViolinPlotStatCompareDataWilcoxTestRefDay0Plot.tiff")
